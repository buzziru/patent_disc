...
**실측 1차 결과**(고정 test 11,271, **KoBERT 재현선** 대비 — 공식 0.8249 아님, 상세 `docs/experiments/modernbert-comparison.md`): A.X-Encoder(8192)가 멀티라벨 micro **0.8502 → 0.8685**(+1.83pt), 앵커 top-1 weighted **0.8148 → 0.8256**으로 재현선을 넘었다. 512 control(exp2)로 분해하면 개선은 **모델·토크나이저 효과 +0.99pt + 컨텍스트 길이 효과 +0.84pt**(대략 절반씩)로 갈리고, 길이 효과는 길이 bin에서 단조 증가(exp1−exp2 micro Δ: B0 +0.46 → B3 +2.64pt)해 장문 가설을 지지한다. **장문 축은 유효하나 단독 원인은 아니며**, headline은 "장문 + 더 나은 한국어 인코더의 결합 효과"로 서술한다.
