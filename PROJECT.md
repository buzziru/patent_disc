...
- **문서당 다중 레이블(multi-label) 188-way 분류.** 한 특허가 여러 중분류에 대응한다(고유 문서의 ~~14%가 2~~10개 `Mno`, 평균 ~1.2개 — `docs/data/data.md`). 188-way **sigmoid + BCE**(`problem_type=multi_label_classification`)로 flat baseline을 세운다. 대분류(`Lno`)는 **예측된 각** `Mno` → `Lno` 매핑으로 유도한다(별도 Lno 헤드 불필요 → 계층 비일관성 회피).
...
**실측 1차 결과**(고정 test 11,271, **KoBERT 재현선** 대비 — 공식 0.8249 아님, 상세 `docs/experiments/modernbert.md`): A.X-Encoder(8192)가 멀티라벨 micro **0.8502 → 0.8685**(+1.83pt), 앵커 top-1 weighted **0.8148 → 0.8256**으로 재현선을 넘었다. 512 control(exp2)로 분해하면 개선은 **모델·토크나이저 효과 +0.99pt + 컨텍스트 길이 효과 +0.84pt**(대략 절반씩)로 갈리고, 길이 효과는 길이 bin에서 단조 증가(exp1−exp2 micro Δ: B0 +0.46 → B3 +2.64pt)해 장문 가설을 지지한다. **장문 축은 유효하나 단독 원인은 아니며**, headline은 "장문 + 더 나은 한국어 인코더의 결합 효과"로 서술한다.
