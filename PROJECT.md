...
- **그 숫자를 절대 기준으로 쓰지 말 것.** baseline의 데이터 분할은 `**documentId`가 train·val 양쪽에 존재하는 데이터 누수** 위험이 있어, 우리는 **누수 없는 데이터셋을 새로 생성**해 비교선을 세운다. 또 `300,240`은 고유 문서 수가 아니라 **(문서,레이블) 쌍 수**이며 우리 로컬 zip은 다른(작은) vintage(고유 224,328건 — `docs/data/data.md`).
- **비교선은 자체 수립**: 우리 데이터에서 **test split을 고정**(재생성 split: train 201,895 / val 11,162 / test 11,271)하고, **KoBERT baseline을 그 test set 위에서 직접 재현**해 비교한다(0.8249 숫자를 그대로 쓰지 않음).
