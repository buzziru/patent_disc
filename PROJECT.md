...
| 갈래 | 결론 | 근거 |
| --- | --- | --- |
| 계층 구조(대분류 조건부) 확장 — **하드 게이트 계보** | 불채택 | 단일 `Lno` 게이트의 micro recall 상한 0.8590 < exp1 현재 recall 0.8697이라 주 지표에서 즉시 손해이고, `Lno`당 단일 `Mno`는 라벨 형상이 배제한다(전체 8.22% 표현 불가). P@1 축에서는 마스킹이 flat과 항등 동치라 이득도 없다 — `docs/experiments/modernbert-comparison.md` 「오류 구조」 |
| 계층 손실(게이트 없는 형태 — 보조 손실) | 불채택 | MCLoss 그룹 항을 앵커에서 손실만 바꿔 집행한 결과 정리 test micro 0.8467 = 앵커 `11_01`(0.8588) 대비 **−1.20pt**(paired bootstrap CI95 [−1.60, −0.80], 시드 잡음의 5.8배)이고 그룹 항이 직접 최적화하는 `Lno` 유도 축도 평탄하다(0.9079 → 0.9074). 기제는 표적 부족이 아니라 전달 실패 — λ를 초기화 손실 **값**으로 고정해 종점 기울기의 67.9%를 그룹 항이 가져갔고(arm은 focal 자체에서도 더 나쁜 지점에 도착), `max` 정의상 형제(within-`Lno`) 밴드가 두 항 어디에도 닿지 않아 조건부 형제 top-1이 −0.53pt로 잡음 밖 하락했다. λ 변형은 훈련 곡선 24지점 중 23지점이 음수라 상한이 "앵커와 같아짐"이고, BCE 기반 교체는 그룹 항 몫을 74.9~80.3%로 키운다. **문헌 사전 확률도 같은 방향이다** — 깊이 2 · 노드 다수 · 훈련 표본 다수 · 불균형 없음은 계층 이득이 사라지는 조건의 조합이고(CoNLL 2024, 같은 조건에서 flat BERT+BCE 대비 최고 계층 모델 +0.19pt), 계층 손실 계열의 이득은 오류 수가 아니라 심각도라 flat leaf micro-F1에 등록되지 않는다(CHAMP). 잔여 파라미터화 계열(조건부 시그모이드·계층 인지 표현)도 이 기대값을 공유한다 — [ADR-0014](docs/adr/0014-hierarchy-loss-closure.md), `docs/experiments/hierarchy-loss.md`「문헌 대조」. **훈련된 조건부 2단계**는 추론 구조 변경을 동반해 위 하드 게이트 행에 걸린다 |
| 카디널리티 회수(손실 · 추론 디코딩) | 회수 불가 | 두 경로가 모두 부정이다. 손실 축은 focal을 넘지 못했고([ADR-0009](docs/adr/0009-loss-axis-closure.md)), 추론 디코딩은 raw 확률 기대-F1 plug-in이 k≥2를 +0.90pt 회수하나 k=1 과대예측과 분리되지 않아 전역 −0.22pt다. 오라클-k +1.60pt는 도달 불가 상한이며 분리에는 문서별 k가 필요한데 그 추정이 닫힌 갈래다 — `docs/experiments/cardinality-decoding.md`(`10_01`) |
...
| 앙상블 | 불채택 | 운영 환경이 단일 모델을 요구한다. 3모델 로짓평균이 앵커 weighted-F1을 0.8256 → 0.8327(+0.71pt)로 올리나 추론 비용이 3배다 — [ADR-0005](docs/adr/0005-no-ensemble.md) |
| 지식 증류(이종 앙상블 → 단일 student) | 불채택 | 앙상블을 훈련 시점 teacher로만 쓰는 경로도 착수 전 게이트로 닫혔다. 무훈련 헤드룸은 실재하고(정리 test micro +0.73pt · k≥2 +1.42pt) **캘리브레이션의 재탕이 아니다**(exp1 재조정의 도달 불가 오라클이 +0.13pt에 그친다 — 새 순위 정보다). 그러나 이득의 형태가 전이에 최악이다: 순 182개 원소가 임계 근처에 몰려 있고(여유 0.2 이상만 채택하면 +0.73pt → +0.01pt), teacher가 갈리는 문서에서 "누가 맞는가"가 길이·k·확신도로 예측되지 않으며(oracle gap 25.11pt 중 0.4~2.1% 회수), 순이득이 104/188 클래스에 흩어져 있다. 계획된 혼합 손실은 λ=0.5가 종점 기울기의 **93%를 distill 항에 주면서** 정작 이득을 만드는 밴드에는 예산의 **1.2%**만 쓰고, 이 쏠림은 λ로 고쳐지지 않는다 — [ADR-0015](docs/adr/0015-kd-closure.md), `docs/experiments/knowledge-distillation.md`「착수 전 게이트 둘」 |
| 라벨 잡음 대응(재라벨링 · 잡음 강건 손실 · 샘플 재가중) | 불착수 | 확신 오답은 데이터가 정한 집합이나(교차 모델 합의 — 앵커 단독 1.6% · KoBERT 81.7% 동조) 그 대응은 열지 않는다. ① 모델 예측으로 정답을 고치는 것은 순환이고 자체 test 재작성은 [ADR-0001](docs/adr/0001-comparison-baseline.md)의 비교선을 무너뜨린다. ② 손실 개입은 부호가 반대다 — γ만 뺀 BCE가 확신 오답을 1.75배 만들고 앵커 집합의 75.3%가 BCE에서도 확신 오답이라, 손실은 질량을 집중시킬 뿐 집합을 만들지 않는다. ③ 잡음 원소가 전체의 0.03%뿐이라 재가중이 붙잡을 질량이 없고 4런 모두 F1이 끝까지 개선해 잡음 암기가 지표를 해친 흔적이 없다. 바뀌는 것은 훈련이 아니라 **한계 기술**이다(위 「스코프·한계」) — [ADR-0016](docs/adr/0016-confident-error-diagnosis.md), `docs/experiments/confident-errors.md` |
...
- **측정 micro-F1은 라벨 잡음만큼 하향 편향돼 있다.** 손실 질량의 73~82%를 무는 확신 오답은 **모델 고유의 실패가 아니라 데이터가 정한 집합**이며(앵커 단독 오답 1.6%, 다른 아키텍처인 KoBERT도 81.7% 동조), 그 상당 부분이 라벨 누락·오부착 후보다 — 확신 FP의 공기 수준이 입력 동일 라벨 충돌 실측([ADR-0010](docs/adr/0010-data-cleaning.md))과 구별되지 않는다. 후보가 전부 실제 잡음이면 편향은 `11_01` +2.14pt · exp1 +1.80pt다(가정적 값). **그만큼의 헤드룸은 애초에 훈련으로 도달할 수 없다** — 더 큰 모델이나 더 긴 컨텍스트로 회수되지 않는다(16배 긴 8192로 훈련한 최고 성능 런도 같은 자리를 81.4% 함께 틀리고, 한 런에서만 나는 오답 24개는 시드 쌍둥이가 100% 맞힌다). 잡음은 특정 중분류가 아니라 **문서 단위로 코퍼스 전반에 얇게** 깔려 있다(충돌 실측 74클래스와 관측/기대 1.08). 정답을 고쳐 다시 재지 않는다 — 결과 보고에 평탄화 분포와 나란히 병기한다([ADR-0016](docs/adr/0016-confident-error-diagnosis.md), 실측 `docs/experiments/confident-errors.md`).
