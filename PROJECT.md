...
- **문서당 다중 레이블(multi-label) 188-way 분류.** 한 특허가 여러 중분류에 대응한다(고유 문서의 ~~14%가 2~~10개 `Mno`, 평균 ~1.2개 — `docs/data/data.md`). 188-way **sigmoid + Focal Loss**(γ=2, `problem_type=multi_label_classification`)로 flat baseline을 세운다. 대분류(`Lno`)는 **예측된 각** `Mno` → `Lno` 매핑으로 유도한다(별도 Lno 헤드 불필요 → 계층 비일관성 회피).
  - 현행 손실 구현의 `alpha`는 손실 전체에 곱해지는 상수라 **클래스 균형 역할을 하지 않는다**(표준 α-balanced focal은 양성·음성에 서로 다른 가중을 준다). 문서당 음성:양성이 약 156:1인데 비대칭 처리는 사실상 γ 하나뿐이다 — 손실 함수는 남아 있는 개선 후보다(계획·후보 평가 `docs/experiments/loss-function.md`).
- **계층 구조(대분류 조건부) 확장은 하지 않는다 — flat 유지.** 오류 분해 실측에서 2단계 추정 P@1(= `Lno` 단계 정확도 × 오라클-`Lno` P@1)이 exp1 **0.8977**로 flat **0.9052**를 밑돈다(3모델 −0.75 ~ −1.07pt). 정답 `Lno`로 열을 제한하면 +5.0pt가 오르지만 `Lno` 단계에서 6.0%를 먼저 잃어 상쇄된다. 형제 혼동 자체는 우연의 5.2배로 잦으나(36.6% vs 우연 7.0%), 그 사실이 조건부 구조의 이득을 뜻하지는 않는다(`docs/experiments/modernbert-comparison.md` 「오류 구조」).
...
| 갈래 | 결론 | 근거 |
| --- | --- | --- |
| 계층 구조(대분류 조건부) 확장 | 불채택 | 2단계 추정 P@1이 flat을 3모델 모두 밑돈다(−0.75~−1.07pt). 오라클-`Lno`로 재면 헤드룸이 +5.0pt이나 `Lno` 단계에서 6.0%를 먼저 잃어 상쇄된다 — `docs/experiments/modernbert-comparison.md` 「오류 구조」 |
...
| 앙상블 | 불채택 | 운영 환경이 단일 모델을 요구한다. 3모델 로짓평균이 앵커 weighted-F1을 0.8256 → 0.8327(+0.71pt)로 올리나 추론 비용이 3배다 |
