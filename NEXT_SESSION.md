...
- **선행 진단 = 8192 LR range test — 준비 완료(`notebook/15_01_LRRangeTest.ipynb`).** [ADR-0011](docs/adr/0011-resource-constrained-methodology.md)「전이 가능한 lr 판정법」이 처방한 horizon-분리 프로브다. 최종 런과 같은 조건(len8192 · eff128 · 정리 데이터)에서 lr을 300 스텝에 걸쳐 1e-6 → 1e-2로 지수 증가시키며 train loss를 관측한다(38,400문서 ≈ 0.19 epoch). 산출은 최적 lr이 아니라 **4.8e-4가 발산 임계 대비 어디에 있는가**다.
  - **판정은 절대값이 아니라 상대 위치로 한다.** `11_01`이 같은 lr로 완주한 **len512를 대조로 함께** 재고(약 7분), 8192에서 운영 lr이 전환점 대비 같은 위치인지 본다 — 경험칙("전환점의 1/3~1/10")을 프로젝트 실측으로 대체한다. 로직은 `src/patent_train/lr_range.py`(`run_lr_range_test`·`compare_lr_range`), 산출은 `output/lr_range_{tag}.json`·`output/lr_range_verdict.json`.
  - **프로브 뒤 모델 가중치는 버린다** — 발산 구간까지 밀어 넣으므로 이어서 훈련하지 않는다.
  - ⚠️ **프로브 config에서 `splits`를 좁히지 않는다.** 훈련 경로는 prep 결과를 `{backbone}_len{max_len}` 캐시에 쓰므로 train만 prep한 캐시가 남으면 **최종 풀런이 val·test 없이 돈다.** 전체 split을 prep해 두면 비싼 8192 prep을 최종 런이 그대로 재사용한다.
