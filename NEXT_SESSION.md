...
| 계층 확장(`Lno` 게이트) | **하드 top-1 게이트 불채택** — 단일 `Lno` 게이트의 micro recall 상한 0.8590이 exp1 현재 recall 0.8697 아래(4모델 동일). 기존 2단계 추정(`Lno` 정확도 × 전체 오라클)은 1단계 실패를 이중 계상한 결함 추정량이고, 조건부로 교정하면 flat과 **항등**이라 판정력이 0이었다. **훈련된 조건부 2단계·소프트(top-m) 게이트는 미측정**(2단계 상한 +3.48pt = sibling 질량, 현행 조건부 정확도 0.9630) | [ADR-0002](docs/adr/0002-flat-multilabel-hierarchy.md) · [modernbert-comparison.md](docs/experiments/modernbert-comparison.md)「오류 구조」(`scripts/hierarchy_conditional.py`) |
