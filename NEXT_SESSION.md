...
**다음 순서는 (1) 시드 분산 측정 1회 → (2) 계층 손실(MCLoss) 1런**이다(「남은 일」 1·2). 시드 분산을 먼저 두는 것은 결과 보고에 훈련 잡음 성분을 넣기로 결정했기 때문이고, 그 값이 나와야 이후 축의 판정선(+0.4pt)이 근거를 갖는다. KD는 그 뒤다([knowledge-distillation.md](docs/experiments/knowledge-distillation.md), 게이트 GREEN이나 GPU 단계가 여럿).
...
1. **시드 분산 측정 1회 — 다음 착수.** 잡음 두 성분 중 **표본 성분은 실측 완료**(고정 test paired bootstrap으로 micro 델타 sd 0.18~0.21pt — [eval-noise.md](docs/experiments/eval-noise.md)), 남은 **훈련(시드) 성분**을 잰다. **결과 보고에 이 성분을 넣기로 결정했다** — 단일 시드 방법론([ADR-0011](docs/adr/0011-resource-constrained-methodology.md))의 방어이자 이후 축 판정선(+0.4pt)의 근거다. 대상은 **`11_01` 재현**(현행 레시피·손실 축 기준선, seed만 교체, A40 ~8.3h)이다 — 여유가 +0.13pt뿐인 focal−BCE가 이 성분에 직접 걸린다. 1회 재현은 sd를 추정하지 못하고 **델타 1개(|Δ|의 규모)** 만 주므로, 보고할 때 그 한계를 명시한다.
2. **계층 손실(MCLoss) 1런.** 표적 질량 실측이 끝나 있고([hierarchy-loss.md](docs/experiments/hierarchy-loss.md)) 추론 구조를 바꾸지 않으므로 앵커 `11_01`에서 손실만 바꾼 단일 변수 대조가 된다. 판정선·잡음 기준은 도메인 축과 같다(+0.4pt, 표본 잡음 sd 0.18~0.21pt).
