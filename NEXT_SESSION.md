...
2. **8192 최종 런 — GPU 작업, 선행 진단 완료.** LR range test가 완주해 신 레시피(eff128 · lr 4.8e-4)의 8192 전이가 안전으로 판정됐다(전환점 대비 14.9배 여유, 512 대조 18.4배). 8192 prep 캐시도 프로브가 만들어 뒀다(「다음 작업」 절).
...
- **레시피 리스크는 해소됐다 — LR range test 완주.** 신 레시피(eff_batch 128 · lr 4.8e-4)는 512에서만 검증됐고 8192에서 돈 적이 없었다(exp1은 구 레시피 eff8 · lr 3e-5). 8192는 같은 eff_batch라도 토큰이 16배라 per-step 기울기 잡음과 발산 임계가 다를 수 있어, [ADR-0011](docs/adr/0011-resource-constrained-methodology.md)「전이 가능한 lr 판정법」이 처방한 horizon-분리 프로브로 잰다. 실행 기록 `notebook_output/15_01_LRRangeTest.ipynb` · SSOT `output/lr_range_verdict.json`.
  ...
  - **판정: 여유 있음 — 신 레시피 그대로 최종 런.** 상대 위치 비 0.806으로 허용치(1/1.5 = 0.67) 안이고, 운영 lr이 양쪽 모두 임계에서 한 자릿수 이상 아래다. 8192 임계가 512보다 19% 낮은 것은 방향은 예상대로이나 폭이 판정을 바꾸지 않는다.
  ...
  - ⚠️ **중단 규칙에 걸리지 않았다**(양쪽 `lr_at_stop: None`). 1e-2까지 훑는 동안 평활 손실이 최저의 4배를 넘지 않아 전환점 이후 상승이 완만했다는 뜻이다. 곡선의 세 구간은 그림으로 확인한다.
  - **프로브 뒤 모델 가중치는 버린다** — 발산 구간까지 밀어 넣으므로 이어서 훈련하지 않는다.
  - ⚠️ **프로브 config에서 `splits`를 좁히지 않았다.** 훈련 경로는 prep 결과를 `{backbone}_len{max_len}` 캐시에 쓰므로 train만 prep한 캐시가 남으면 **최종 풀런이 val·test 없이 돈다.** 전체 split을 prep해 뒀으니 **비싼 8192 prep을 최종 런이 그대로 재사용한다.**
  - **프로브 로직은 패키지에 없다** — `notebook_output/15_01_LRRangeTest.ipynb`가 `TrainingRunner.build_trainer`만 오버라이딩해 노트북 안에 둔다(`14_01`이 `FocalLoss`를 상속했던 규약). `src/patent_train`은 검증된 코드만 유지한다.
