...
>
> ⚠️ **집행은 후순위다.** 게이트가 GREEN이고 프로토콜도 확정됐으나 순서를 뒤로 둔다 — 앞선 축은 도메인 사전학습(TAPT) 분류 판정(`domain-pretraining.md`)과 계층 손실(`hierarchy-loss.md`)이다. KD는 teacher 로짓 생성부터 student 훈련까지 GPU 잡이 여러 단계인 반면 두 축은 각각 1런이고, 표적도 겹치지 않는다(KD는 k≥2 카디널리티, 계층 손실은 cross-`Lno` 오류).
