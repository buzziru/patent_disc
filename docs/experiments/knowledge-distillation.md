...
>
> **축은 종결됐다**([ADR-0015](../adr/0015-kd-closure.md)). 헤드룸 게이트는 GREEN이나(증류할 상한 +0.73pt 실재), 그 뒤에 집행한 **전이 구조 게이트**와 **기울기 예산 게이트**가 상한의 형태와 전달 경로를 각각 기각했다 — 「착수 전 게이트 둘」 절. 아래 계획·프로토콜은 집행되지 않은 설계로 남으며, 실측 SSOT는 `output/kd_gate_ensemble.json`·`output/kd_transfer_structure.json`·`output/kd_grad_budget.json`이다.
...
**B. 축퇴는 test에서 관측되지 않는다.** `BCE(z,q)`와 `BCE(z,y)`의 기울기 코사인이 0.354(`11_01`) / 0.258(exp1)이고 L1 차가 기준의 217% / 287%다 — 종점 모델이 test에서 이미 `q`보다 하드 라벨에 가까워 두 목적이 확연히 다르다. 문서당 중간대 2.01개라는 통계가 과소평가한 값이다. ⚠️ 단 판정 축은 train이며, teacher가 암기한 train 위에서는 `q → y`로 수렴해 축퇴가 심해진다(「선행 작업」 2가 그 측정이다).
