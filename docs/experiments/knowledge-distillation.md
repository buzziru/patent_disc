...
- **soft target 배합이 작동점 레버다.** best(exp1 상향)는 전역 micro 최고, 등가중·로짓평균은 k≥2 최고 — 증류 타깃을 어떻게 짜느냐로 student가 k=1↔k≥2 트레이드 위 어디에 앉을지 조절된다.
...
| **ASL** | `modernbert-patent-len512-asl` | k≥2 recall 성분(과소예측 억제) | k≥2 micro 0.8044·FN 최저, 작동점 다양성 |
...
- **λ=1.0(순수 증류) 스윕의 위험이 구체화된다.** `q ≈ y`인 구간에서 λ=1.0은 하드 라벨 위 BCE로 무너지고, BCE는 focal 대비 열세로 이미 측정됐다([ADR-0009](../adr/0009-loss-axis-closure.md)). 정보량 확인용이라는 명분과 별개로 성능은 후퇴가 기대값이다.
...
- **혼합 손실**: `L = (1−λ)·L_hard + λ·L_distill`.
  - `L_hard` = focal(α=0.25, γ=2), 정답 다중-핫 `y` — 프로젝트 확정 손실.
...
**B. 축퇴는 test에서 관측되지 않는다.** `BCE(z,q)`와 `BCE(z,y)`의 기울기 코사인이 0.354(`11_01`) / 0.258(exp1)이고 L1 차가 기준의 217% / 287%다 — 종점 모델이 test에서 이미 `q`보다 하드 라벨에 가까워 두 목적이 확연히 다르다. 문서당 중간대 2.01개라는 통계가 과소평가한 값이다. 단 판정 축은 train이며, teacher가 암기한 train 위에서는 `q → y`로 수렴해 그 경향이 심해진다(「선행 작업」 2가 그 측정이다).
...
**미측정으로 남는 것**: (1) 2048의 길이 부채(통제군 집행하지 않음) (2) train 위 `q`의 포화도. 둘 다 KD 재개의 전제가 아니라 배포 모델 길이 선택과 얽힌 양이다.
