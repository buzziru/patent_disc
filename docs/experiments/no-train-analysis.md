...
- **판정: flat 유지.** 정답 `Lno`로 열을 제한하면 exp1 P@1이 0.9052 → 0.9551(+5.0pt)로 올라 조건부 구조의 이득 자체는 실재한다. 그러나 `Lno` 단계에서 6.0%를 먼저 잃기 때문에 2단계 추정은 **0.8977로 flat보다 낮다**. 3모델 전부 음수(−0.75 ~ −1.07pt)로 방향이 같다.
- ⚠️ **추정치의 한계**: 조건부 단계를 재훈련하지 않고 flat 로짓을 마스킹해 시뮬레이션한 값이며, 2단계 곱은 두 단계의 독립을 가정한다. 실제 조건부 단계는 마스킹보다 나을 수 있다. 방향은 강하나 증명이 아니다. 다만 `PROJECT.md`가 독립된 두 헤드를 금지하므로 `Lno` 단계를 별도로 강화하는 경로는 이미 닫혀 있다.
...
- **형제 혼동은 우연보다 5배 잦다 — 그러나 이는 서술 통계다.** 오답을 비정답 클래스에서 균등 추출하면 sibling 확률은 7.0%인데 실측은 36.6%다. 모델이 형제를 특별히 헷갈린다는 사실은 성립하지만, 그것이 계층 구조의 이득을 뜻하지는 않는다 — 판정은 위의 2단계 추정으로 내린다. sibling 비율만 보고 "형제 혼동이 드물다"로 읽지 않도록 우연 수준을 함께 둔다.
...
- **상위 쌍의 대칭성은 추가 레버가 아니다** — exp1 상위 5쌍(EA↔EI 27/26 대칭도 0.963, EA↔ND 15/22, EH↔ND 14/21, LB↔LC 16/19, EH↔EI 20/13)이 모두 양방향이나, 혼동 가능하고 크기가 비슷한 두 대분류에서 오류가 양방향으로 흐르는 것은 귀무 기대값이다. 국소 보정이 가능한 쪽은 비대칭 쌍인데 상위 쌍 대칭도가 0.65~0.96으로 전부 대칭이다. 쌍을 완벽히 해소한 상한도 상위 1쌍 +0.42pt / 5쌍 +1.35pt / 10쌍 +2.01pt로 전체 오라클-`Lno` +5.0pt의 부분집합이며, 쌍 전문가는 계층 확장과 동일한 게이트 비용을 물기 때문에 **판정은 위와 같다(flat 유지).** 쌍 단위 국소 처리는 새 갈래가 아니다.
...
- **B의 τ 정책은 이 헤드룸에 닿지 못한다** — 단일 전역 τ는 문서별 카디널리티를 표현할 수 없다(B 실측 global τ의 micro 오라클 헤드룸 +0.0000~+0.0008). 회수 경로는 손실 함수이며 `../../NEXT_SESSION.md` 2단계의 정량 목표로 반입했다.
...
세 분석이 다음 훈련의 해석 틀을 제공한다 — 오류 구조 기준선, 3모델 공통 τ 정책, 토크나이저 커버리지 보정. 다음 훈련은 레시피 확정(배치·LR)이고 그 뒤가 손실 함수이며, KLUE-RoBERTa-base 대조군은 선택 항목으로 밀렸다(`../../NEXT_SESSION.md`).
