...
일반 도메인 백본(`skt/A.X-Encoder-base`)을 자체 특허 코퍼스로 MLM 계속학습(TAPT)한 뒤 분류 파인튜닝해, **백본 표현 품질이 남은 레버인지**를 재는 축이다. 검토 후 폐기한 대안(기성 특허 백본·토크나이저 갈래·임베딩-only 계속학습)은 아래 「검토 후 불채택」이 소유한다.
