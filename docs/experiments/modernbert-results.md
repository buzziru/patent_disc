...
두 축은 계산이 달라 서로 뺄셈하지 않고 각 축에서 비교한다. exp1이 **두 축 모두** baseline을 이겼다(headline: 다중 라벨 micro +1.8pt, 기준 런 +1.1pt). (참고: 기준 런 0.8256이 공식 0.8249를 넘지만 **서로 다른 test set**이라 직접 비교 대상 아님.)
