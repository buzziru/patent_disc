...
- **개선은 길이와 모델이 각각 절반가량 기여한다(exp2 control로 분해 확정).** 전체 micro +1.83pt = 모델·토크나이저 효과 +0.99pt(exp2 vs KoBERT, 둘 다 512) + 컨텍스트 길이 효과 +0.84pt(exp1 vs exp2, 둘 다 A.X). 상세는 [`modernbert-comparison.md`](./modernbert-comparison.md) 「길이 vs 모델 분해」.
