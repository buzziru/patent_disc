...
- **argmax 보정 멀티라벨**(빈 예측 문서에 argmax 1개 강제): micro 0.8675 / macro 0.8641 / sample 0.8871 — keep 대비 sample만 소폭 상승(+0.0045), 빈 예측 1.34%가 지표에 큰 왜곡을 주지 않음을 확인.
- **앵커 p@k**: p@1 0.9053 / p@3 0.9705 / p@5 0.9829.

exp1 자체 bin은 B0(micro 0.8765) → B3(0.8490)로 완만히 하락한다. 이 표는 **bin별 Δ(A.X − KoBERT)** 산출(길이 vs 모델 판별, 「핵심 비교 축」)의 A.X 쪽 입력이며, KoBERT bin 대조는 exp2와 함께 확정한다.
...
- **개선은 길이와 모델이 각각 절반가량 기여한다(exp2 control로 분해 확정).** 전체 micro +1.83pt = 모델·토크나이저 효과 +0.99pt(exp2 vs KoBERT, 둘 다 512) + 컨텍스트 길이 효과 +0.84pt(exp1 vs exp2, 둘 다 A.X). 길이 효과는 bin에서 단조 증가(exp1−exp2 micro Δ: B0 +0.46 → B3 +2.64pt)로 확증되고, 모델 효과는 절단 없는 B0에서도 exp2가 KoBERT를 넘는 것으로 확인된다. 상세는 「길이 vs 모델 분해」 절.
